...
export abstract class Model {
  ...
  public field(channel: Channel, opt: FieldRefOption = {}) {
    ...
    if (fieldDef.bin) { // bin has default suffix that depends on scaleType
      opt = extend({
        binSuffix: hasDiscreteDomain(this.getScale(channel).type) ? 'range' : 'start'
      }, opt);
    }
  ...
  public getScale(channel: Channel): Scale {
    return this.scales[channel];
  }

  public hasDiscreteScale(channel: Channel) {
    const scale = this.getScale(channel);
  ...
  public scaleName(this: Model, originalScaleName: Channel|string, parse?: boolean): string {
    ...
    if (
        ...
        (this.getScale && this.scales[channel]) ||
  ...
  public getAxis(channel: Channel): Axis {
    return this.axes[channel];
  }

  public getLegend(channel: Channel): Legend {
