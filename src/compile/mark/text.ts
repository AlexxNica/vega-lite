...
export const text: MarkCompiler = {
  ...
  encodeEntry: (model: UnitModel) => {
    ...
    e.x = ref.stackable(X, encoding.x, model.scaleName(X), model.getScale(X), stack, xDefault(config, textDef));
    e.y = ref.stackable(Y, encoding.y, model.scaleName(Y), model.getScale(Y), stack, ref.midY(config));

    e.fontSize = ref.midPoint(SIZE, encoding.size, model.scaleName(SIZE), model.getScale(SIZE),
