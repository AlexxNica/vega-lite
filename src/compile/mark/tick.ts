...
export const tick: MarkCompiler = {
  ...
  encodeEntry: (model: UnitModel) => {
    ...
    e.xc = ref.stackable(X, encoding.x, model.scaleName(X), model.getScale(X), stack, ref.midX(config));
    e.yc = ref.stackable(Y, encoding.y, model.scaleName(Y), model.getScale(Y), stack, ref.midY(config));

    if (config.mark.orient === 'horizontal') {
      e.width = size(encoding.size, model.scaleName(SIZE), model.getScale(SIZE), config, (model.getScale(X) || {}).rangeStep);
      e.height = {value: config.tick.thickness};
    } else {
      e.width = {value: config.tick.thickness};
      e.height = size(encoding.size, model.scaleName(SIZE), model.getScale(SIZE), config, (model.getScale(Y) || {}).rangeStep);
    }
