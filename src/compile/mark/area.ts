...
export const area: MarkCompiler = {
  ...
  encodeEntry: (model: UnitModel) => {
    ...
    if (orient === 'horizontal') {
      e.x2 = ref.stackable2(X2, encoding.x, encoding.x2, model.scaleName(X), model.getScale(X), stack, 'base');
    } else {
      e.y2 = ref.stackable2(Y2, encoding.y, encoding.y2, model.scaleName(Y), model.getScale(Y), stack, 'base');
    }
