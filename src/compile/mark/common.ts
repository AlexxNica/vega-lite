...
export function applyColorAndOpacity(e: VgEncodeEntry, model: UnitModel) {
  ...
  let colorRef = ref.midPoint('color', model.encoding.color, model.scaleName('color'), model.getScale('color'), undefined);
  let opacityRef = ref.midPoint('opacity', model.encoding.opacity, model.scaleName('opacity'), model.getScale('opacity'), config.mark.opacity && {value: config.mark.opacity});
