...
function x(model: UnitModel, stack: StackProperties) {
  ...
  if (orient === 'horizontal') {
    e.x = ref.stackable(X, xDef, xScaleName, model.getScale(X), stack, 'base');
    e.x2 = ref.stackable2(X2, xDef, encoding.x2, xScaleName, model.getScale(X), stack, 'base');
    return e;
  } else { // vertical
    if (isFieldDef(xDef)) {
      if (xDef.bin && !sizeDef) {
    ...
    e.xc = ref.midPoint(X, xDef, xScaleName, model.getScale(X),
      extend(ref.midX(config), {offset: 1}) // TODO: config.singleBarOffset
    );
    e.width = ref.midPoint(SIZE, encoding.size, model.scaleName(SIZE), model.getScale(SIZE),
      defaultSizeRef(xScaleName, model.getScale(X), config)
    );
    return e;
  }
}
...
function y(model: UnitModel, stack: StackProperties) {
  ...
  if (orient === 'vertical') {
    e.y = ref.stackable(Y, yDef, yScaleName, model.getScale(Y), stack, 'base');
    e.y2 = ref.stackable2(Y2, yDef, encoding.y2, yScaleName, model.getScale(Y), stack, 'base');
    return e;
  } else {
    if (isFieldDef(yDef)) {
      if (yDef.bin && !sizeDef) {
    ...
    e.yc = ref.midPoint(Y, yDef, yScaleName, model.getScale(Y),
      ref.midY(config)
    );
    e.height = ref.midPoint(SIZE, encoding.size, model.scaleName(SIZE), model.getScale(SIZE),
      defaultSizeRef(yScaleName, model.getScale(Y), config)
    );
    return e;
  }
}
